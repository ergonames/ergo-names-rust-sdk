...
pub fn get_token_transaction_data(token_id: &str) -> Result<Value> {
    ...
    let url: String = format!("{}api/v1/transactions/search?query={}&offset={}", consts::EXPLORER_API_URL, token_id, total-1);
    let resp: String = reqwest::blocking::get(url)?.text()?;
    let data: Value = serde_json::from_str(&resp)?;
...
pub fn get_single_transactions_for_token(token_id: &str) -> Result<Value> {
    let url: String = format!("{}api/v1/transactions/search?query={}&limit=1", consts::EXPLORER_API_URL, token_id);
    let resp: String = reqwest::blocking::get(url)?.text()?;
    let data: Value = serde_json::from_str(&resp)?;
    return Ok(data);
}